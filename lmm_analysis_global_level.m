function [p, r2_adj, BIC, r] = lmm_analysis_global_level(ConMats, SubjRecs, RecAges, LMM_order)
%LMM_ANALYSIS_GLOBAL_LEVEL Global-level statistical test with linear mixed effects model (LMM)
...
%   consecutively. ConMat may have NaN on diagonal.
...
K = zeros(2,N_Fc);
    ...
    for f = 1:N_Fc
        ...
        Connectivity = squeeze(mean(mean(connectivity_matrices, 'omitnan'), 'omitnan')); 
